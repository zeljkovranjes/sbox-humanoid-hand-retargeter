// Reused from humanoid-retargeter 26084c96c3fc870aaf9a5bd798de063ce2fd62df; shared validation and continuity added.
...
namespace HumanoidHandRetargeter.Formats.Dmx;
...
public sealed class DmxWriteOptions
{
    ...
    /// (fbx2dmx writes the source .fbx path here). Path separators are normalized to '/'.</summary>
...
public static class DmxWriter
{
    ...
    public static string Write(SkeletonModel skeleton, Clip clip, DmxWriteOptions options)
    {
        ...
        if (skeleton.Count == 0)
            throw new ArgumentException("Target skeleton has no bones.", nameof(skeleton));
        if (clip.FrameCount == 0)
            throw new ArgumentException("Clip has no frames.", nameof(clip));
        var restErrors = PoseValidator.Validate(skeleton, Pose.Rest(skeleton));
        if (restErrors.Count > 0) throw new RigValidationException(restErrors);
        if (options.ChannelExcludedBones is not null)
            foreach (var bone in options.ChannelExcludedBones)
                if (bone < 0 || bone >= skeleton.Count)
                    throw new ArgumentOutOfRangeException(nameof(options), $"Excluded bone index {bone} is outside the target skeleton.");
        for (var f = 0; f < clip.FrameCount; f++)
        {
            if (clip.Frames[f].Length != skeleton.Count)
                throw new ArgumentException(
                    ...
                    nameof(clip));
            var errors = PoseValidator.Validate(skeleton, new Pose(clip.Frames[f]));
        ...
        w.Attr("name", "string", options.SourceNote.Replace('\\', '/'));
        w.EndArrayElement(last: true);
        w.EndArray();
        w.EndInlineAttr();
        ...
        string transformGuid, bool position, bool last)
    {
        ...
        for (var f = 0; f < clip.FrameCount; f++)
        {
            ...
            {
                var q = x.Rot;
                if (f > 0) q = QuaternionContinuity.Align(prev, q);
                prev = q;
                value = Quat(q);
            }
    ...
    private sealed class Emitter
    {
        ...
        public void Attr(string name, string type, string value)
        {
            ...
            // The inherited emitter writes literal KV2 fields. Fail clearly on names
            // it cannot represent, instead of producing a malformed animation asset.
            if (value.Any(c => c == '"' || c == '\\' || char.IsControl(c)))
                throw new ArgumentException($"DMX {name} contains an unsupported quote, backslash or control character.");
