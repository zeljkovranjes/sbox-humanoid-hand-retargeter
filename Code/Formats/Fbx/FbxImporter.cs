...
public static class FbxImporter
{
    ...
    /// ancestors (Null/Root containers included), plus every animated model and its ancestors.
    /// This retains animated weapon meshes and camera/helper tracks alongside arms.
    /// Unanimated mesh leaves are excluded; last resort, all non-Mesh models.
    ...
    private static List<FbxObject> SelectSkeletonModels(FbxScene scene)
    {
        ...
        // FPS files can animate weapon meshes, cameras and helper roots alongside
        // limbs. Keep those transforms and their ancestry even when limbs exist.
        {
