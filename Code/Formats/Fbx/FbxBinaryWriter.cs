// Reused from the sibling humanoid-retargeter FBX import pipeline.
...
/// <summary>
/// Serializes an <see cref="FbxNode"/> tree back to binary FBX using the 7.x u32
/// header layout (preserving older versions, capped at 7400). The inverse of
...
/// retains the source document's watermark, followed by alignment padding, the version
/// echo, 120 zero bytes and the closing magic. The watermark is tied to document
/// metadata; replacing it can make the native importer silently return an empty model.
/// </remarks>
public static class FbxBinaryWriter
{
    ...
    public static byte[] Write(FbxNode root)
    {
        ...
        var sourceVersion = root.Child("FBXHeaderExtension")?.Child("FBXVersion")?.Prop<int>(0) ?? 7400;
        var version = sourceVersion < 7500 ? (uint)sourceVersion : Version;
        ...
        WriteFooter(ms, version, root.FooterWatermark);
        return ms.ToArray();
    }
    ...
    private static void WriteNode(MemoryStream ms, FbxNode node)
    {
        ...
        if (node.Children.Count > 0 || node.HasChildScope || node.Properties.Count == 0)
        {
    ...
    private static void WriteFooter(MemoryStream ms, uint version, byte[]? watermark)
    {
        ms.Write(watermark is { Length: 16 } ? watermark : FooterWatermark);
